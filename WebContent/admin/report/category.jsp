            ...
            <table class="table table-hover">
                ...
                <tbody>
                    <%
                    	...
                    	PreparedStatement ps = con.prepareStatement("select id,title,description,open,dept_id,technician_id,user_id,close,nvl(trunc(close-open), -1) from complaints where category_id=? order by id desc");
                        ps.setInt(1, Integer.parseInt(request.getParameter("id")));
                    	ResultSet rs = ps.executeQuery();
                    	while (rs.next())
                        {
                            PreparedStatement ps2 = con.prepareStatement("select name from users where id=?");
                            ps2.setInt(1, rs.getInt(6));
                            ResultSet rs2 = ps2.executeQuery();
                            String technician = "";
                            if(rs2.next()) {
                            	technician = rs2.getString(1);
                            }
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br><b>Technician:</b> <a target="_blank" href="<%= request.getContextPath() %>/admin/report/technician.jsp?id=<%= rs.getInt(6) %>"><%= technician %></a>
                            </td>
                            <td><%= rs.getDate(4) %></td>
                            <td><%= rs.getDate(8) %></td>
                            <td><%
